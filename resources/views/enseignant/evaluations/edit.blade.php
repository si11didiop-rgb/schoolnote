        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-green-500 to-green-600 px-6 py-4">
                    <h3 class="text-white font-bold text-lg">✏️ Modifier l'évaluation</h3>
                    <p class="text-green-100 text-sm mt-1">{{ $evaluation->titre }}</p>
                </div>

                <div class="p-6">
                    <form action="{{ route('enseignant.evaluations.update', $evaluation) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select id="enseigner_id" name="enseigner_id" required
                                ...
                                @foreach ($enseignements as $enseignement)
                                    <option value="{{ $enseignement->id }}"
                                        @selected(old('enseigner_id', $evaluation->enseigner_id) == $enseignement->id)>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="type" name="type" required
                                    class="block mt-1 w-full border-gray-300 rounded-lg shadow-sm">
                                @foreach (['Contrôle', 'Devoir surveillé', 'Interrogation', 'Dissertation'] as $typeOption)
                                    <option value="{{ $typeOption }}"
                                        @selected(old('type', $evaluation->type) == $typeOption)>
                                        {{ $typeOption }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="titre" name="titre" class="block mt-1 w-full rounded-lg"
                                          type="text" :value="old('titre', $evaluation->titre)" required />
                        ...
                        <div class="mb-4 grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="heure_fin" name="heure_fin" class="block mt-1 w-full rounded-lg"
                                              type="time"
                                              :value="old('heure_fin', $evaluation->heure_fin)"
                                              required min="07:30" max="18:00" />
